...
### Live-board review (2026-07-05): owner-spotted, F45 family

| ID | Sev | Finding | Status |
|---|---|---|---|
| F45 | MED-HIGH | Same-series next-day cards inherited today's splits/status/grade via the dateless (away,home) join; phantom next-day sharp recs possible on evening boards | FIXED: 12h start-time proximity required on every AN join; wrong-day chips cleared |
| F45b | LOW | Rec chips displayed on live/final games (live -1100 shown as a bettable rec) | FIXED: recs build only clock-pregame |
| F45c | MED | F37's an_ml capture never persisted (splits-only map passed into analyze_game); dead since shipping. Third F35-class boundary miss | FIXED at the main-loop attachment; test now crosses log_snapshots |
| F45d | - | All-D board verdict: WORKING AS DESIGNED. Holiday slate had public and money aligned on every splits game (tickets 50-86 on the money side), zero contrarian, zero steam, and all value EVs negative (-3.5 to -6.5). A disciplined tool outputs nothing on such a night. Thresholds untouched per F33; grade distribution joins the gated review at 100 plays | CLOSED |
